...
import getResolvedTrackingAddress from '@salesforce/apex/OrderTrackerController.getResolvedTrackingAddress';
...
export default class OrderTracker extends LightningElement {
    @api recordId;
    /** Injected on record pages; optional in Experience Builder. */
    @api objectApiName;
    actualOrderId = null;
    ...
    // Optional override: destination from Experience Builder (expressions or static text).
    // Strong fields (street / city / postal) skip Apex; country- or state-only still loads Apex and merges per field.
    ...
    get shippingStreet() {
        return this.normalizeExpressionValue(this.destinationStreet) || this.orderData?.shippingStreet;
    }
    get shippingCity() {
        return this.normalizeExpressionValue(this.destinationCity) || this.orderData?.shippingCity;
    }
    get shippingState() {
        return this.normalizeExpressionValue(this.destinationState) || this.orderData?.shippingState;
    }
    get shippingPostalCode() {
        return this.normalizeExpressionValue(this.destinationPostalCode) || this.orderData?.shippingPostalCode;
    }
    get shippingCountry() {
        return this.normalizeExpressionValue(this.destinationCountry) || this.orderData?.shippingCountry;
    }
    get status() { return this.orderData?.status || 'In Transit'; }

    get addressSourceLabel() {
        const src = this.orderData?.addressSource;
        return src && src !== 'None' ? src : '';
    }

    /** Use coordinates returned from Salesforce when present (avoids Apex HTTP geocode on Experience sites). */
    get recordDestinationLatLng() {
        const d = this.orderData;
        if (!d) {
            return null;
        }
        const lat = Number(d.shippingLatitude);
        const lng = Number(d.shippingLongitude);
    ...
    /**
     * Street, city, or postal from Builder: treat as full manual address (skip Apex).
     * Country- or state-only does not qualify, so resolution still runs and merges.
     */
    get hasStrongDestinationOverride() {
        return !!(
            this.normalizeExpressionValue(this.destinationStreet) ||
            this.normalizeExpressionValue(this.destinationCity) ||
            this.normalizeExpressionValue(this.destinationPostalCode)
        );
    }

    /** Show address/status when Apex returned data or Builder supplied a strong (geocodable) override. */
    get hasShippingInfoPanel() {
        return this.orderData != null || this.hasStrongDestinationOverride;
    }
    ...
    async connectedCallback() {
        ...
        // Strong Builder fields: skip Apex (manual full address). Country/state alone still loads Apex below.
        if (this.hasStrongDestinationOverride) {
            console.log('Using full destination override from Experience Builder; skipping Apex address resolution');
    ...
    /**
     * When Apex HTTP callouts fail (guest / Remote Site), try OpenStreetMap from the browser.
     * Site CSP must allow https://nominatim.openstreetmap.org if this is blocked.
     */
    async tryClientGeocodeDestination() {
        const parts = [
            this.shippingStreet,
        ...
        try {
            ...
            console.warn('Client-side geocode failed (CSP or network):', e);
        }
        return null;
    }
    ...
    async loadOrderData() {
        ...
        try {
            this.orderData = await getResolvedTrackingAddress({ recordId: this.actualOrderId });
            console.log('Resolved tracking address:', this.orderData);
        } catch (error) {
            console.error('Error loading resolved address:', error);
            this.error = true;
    ...
    renderedCallback() {
        ...
        // BUT: Skip this wait if we have a full manual override (no Apex load expected)
        if (!this.hasStrongDestinationOverride && this.actualOrderId && !this.orderData && !this.error) return;
        ...
        if (this.leafletLoadScheduled) {
            return;
        }
        this.leafletLoadScheduled = true;
        Promise.all([
    ...
    async geocodeAddresses() {
        ...
        try {
            ...
            // Prefer Salesforce lat/lng on Shipment/Order/Account (works when callouts fail in Experience Cloud)
            const hasDestLines = !!(
                this.shippingStreet ||
                this.shippingCity ||
                this.shippingPostalCode ||
            ...
            const recordLatLng = this.recordDestinationLatLng;
            if (recordLatLng) {
                this.destinationCoords = recordLatLng;
            } else if (hasDestLines) {
                ...
                try {
                    ...
                    if (destinationResult.success) {
                        this.destinationCoords = [Number(destinationResult.latitude), Number(destinationResult.longitude)];
                    } else {
                        console.warn('Server geocoding failed:', destinationResult.errorMessage);
                        const clientCoords = await this.tryClientGeocodeDestination();
                        if (clientCoords) {
                            this.destinationCoords = clientCoords;
                        } else {
                            console.warn('Client geocode unavailable; using offset from warehouse as last resort');
                            this.destinationCoords = [
                                this.warehouseCoords[0] + 0.12,
                                this.warehouseCoords[1] + 0.12
                            ];
                        }
                    }
                } finally {
                ...
                // Demo mode - default pin offset from warehouse defaults
